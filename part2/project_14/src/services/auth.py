...
class Auth:
    """
    A class that handles authentication and authorization in a FastAPI application.

    This class includes methods for password hashing and verification, token creation and decoding,
    and user retrieval from the database. It also includes a `RoleAccess` class for managing role-based
    access control.

    Attributes:
        pwd_context (CryptContext): The cryptographic context for password hashing and verification.
        ...
        cache (Redis): The Redis cache instance for storing user data.
        oauth2_scheme (OAuth2PasswordBearer): The OAuth2 password bearer instance for token retrieval.
    """
    ...
    def get_password_hash(self, password: str):
        """
        Generate a hashed password from the plain password.
    ...
    async def create_access_token(
        ...
        :param expires_delta: The number of seconds the token should be valid for.
    ...
    async def get_email_from_token(self, token: str):
        """
        Decode an email token and return the email.
